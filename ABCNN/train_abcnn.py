...
tf.flags.DEFINE_string("distance_type", "euclidean", "Type of distance calculation (euclidean, cos, dot)")
        ...
        with sess.as_default():
            ...
            with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
                learning_rate = tf.train.exponential_decay(learning_rate=FLAGS.learning_rate,
                                                           global_step=abcnn.global_step, decay_steps=FLAGS.decay_steps,
            ...
            acc_summary = tf.summary.scalar("accuracy", abcnn.accuracy)
            ...
            def train_step(x_batch_front, x_batch_behind, y_batch):
                ...
                _, step, summaries, loss, accuracy = sess.run(
                    [train_op, abcnn.global_step, train_summary_op, abcnn.loss, abcnn.accuracy], feed_dict)
            ...
            def validation_step(x_batch_front, x_batch_behind, y_batch, writer=None):
                ...
                step, summaries, loss, accuracy, recall, precision, f1, auc = sess.run(
                    [abcnn.global_step, validation_summary_op, abcnn.loss, abcnn.accuracy,
                     abcnn.recall, abcnn.precision, abcnn.F1, abcnn.AUC], feed_dict)
